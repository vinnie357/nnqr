...
fn test_movement_indicator_board_alignment() {
    ...
    println!("   BOARD_WIDTH: {}, BOARD_HEIGHT: {}", BOARD_WIDTH, BOARD_HEIGHT);
    ...
    for &(x, y) in &test_positions {
        ...
        println!("   Board({}, {}) -> World({:.2}, {:.2}, {:.2})", 
                 x, y, world_pos.x, world_pos.y, world_pos.z);
        println!("      Centered: ({:.2}, {:.2}) -> Iso: ({:.2}, {:.2})", 
    ...
    println!("   Board center: ({:.1}, {:.1})", board_center_x, board_center_y);
    ...
    for &(x, y) in &center_tiles {
        ...
        println!("   Tile({}, {}) distance from origin: {:.2}", x, y, distance_from_origin);
    }
    
    // Check if coordinate system is consistent
    println!("\n✅ Coordinate System Verification:");
    println!("   Both board tiles and movement indicators use board_to_isometric()");
    println!("   Both use same enhanced_tile_size: {:.2}", enhanced_tile_size);
    ...
    for &move_pos in &move_positions {
        ...
        let distance = ((move_world.x - piece_world.x).powi(2) + 
                       (move_world.z - piece_world.z).powi(2)).sqrt();
        println!("   Move to {:?} -> World({:.2}, {:.2}, {:.2}), Distance: {:.2}", 
    ...
    println!("   Expected distance for adjacent moves: ~{:.2}", expected_distance);
}