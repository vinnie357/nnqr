...
fn test_occupied_tile_blocking() {
    ...
    let _piece1 = world.spawn(GamePiece {
        player: Player::Player1,
        board_position: (0, 0),
    }).id();
    
    let _piece2 = world.spawn(GamePiece {
        player: Player::Player2,
        board_position: (0, 1),
    }).id();
    
    let _piece3 = world.spawn(GamePiece {
        player: Player::Player1,
        board_position: (1, 0),
    }).id();
    
    // Cannot move to tile occupied by same player
    assert!(!can_move_to_occupied_tile(&mut world, (0, 0), (1, 0), Player::Player1));
    
    // Can capture opponent piece
    assert!(can_move_to_occupied_tile(&mut world, (0, 0), (0, 1), Player::Player1));
}
...
fn can_move_to_occupied_tile(world: &mut World, _from: (u8, u8), to: (u8, u8), player: Player) -> bool {
    // Check all pieces
