...
fn test_complete_2d_drag_workflow_all_pieces() {
    ...
    for y in 0..2 {  // Bottom two rows
        for x in 0..BOARD_WIDTH {
            if (x + y) % 2 == 0 {  // Checkerboard pattern
                ...
                println!("   Piece at board ({}, {}) -> world ({:.1}, {:.1})", x, y, world_x, world_y);
            }
        }
    }
    ...
    for y in (BOARD_HEIGHT - 2)..BOARD_HEIGHT {  // Top two rows
        for x in 0..BOARD_WIDTH {
            if (x + y) % 2 == 0 {  // Checkerboard pattern
                let enhanced_tile_size = TILE_SIZE * 1.2;
                let world_x = (x as f32 - BOARD_WIDTH as f32 / 2.0 + 0.5) * enhanced_tile_size;
...
fn test_player_pieces(
    ...
    for (i, (entity, board_pos, world_pos)) in pieces.iter().enumerate() {
        println!("   Testing {:?} piece {} at board {:?}, world ({:.1}, {:.1})", 
                 player, i + 1, board_pos, world_pos.x, world_pos.y);
        ...
        let indicators_before = app.world.query::<&ValidMoveIndicator>().iter(&app.world).count();
        ...
        let indicators_after = app.world.query::<&ValidMoveIndicator>().iter(&app.world).count();
        ...
        if should_be_movable {
            ...
            if indicators_spawned {
                println!("     ✅ Valid move indicators spawned ({} indicators)", indicators_after - indicators_before);
            } else {
                println!("     ⚠️ No valid move indicators spawned");
            }
            
            assert!(piece_was_selected, 
                "Piece {} should be selectable on {:?}'s turn", i + 1, player);
        } else {
            if !piece_was_selected {
            ...
            assert!(!piece_was_selected, 
                "Piece {} should NOT be selectable on opponent's turn", i + 1);
        }
...
fn test_hit_detection_precision(
    ...
    println!("       Center ({:.1}, {:.1}): {}", center_click.x, center_click.y, 
             if center_result { "HIT" } else { "MISS" });
    println!("       Edge ({:.1}, {:.1}): {}", edge_click.x, edge_click.y, 
             if edge_result { "HIT" } else { "MISS" });
    println!("       Outside ({:.1}, {:.1}): {}", outside_click.x, outside_click.y, 
    ...
    assert!(!outside_result, "Outside click should miss piece {}", piece_num);
}
...
fn cleanup_test_state(app: &mut App) {
    ...
    let dragging_entities: Vec<Entity> = app.world.query::<Entity>()
        .iter(&app.world)
...
fn test_complete_drag_lifecycle() {
    ...
    let indicator_count = app.world.query::<&ValidMoveIndicator>().iter(&app.world).count();
    
    println!("   Piece has Dragging component: {}", has_dragging);
    println!("   Valid move indicators spawned: {}", indicator_count);
    
    assert!(has_dragging, "Piece should have Dragging component after click");
    ...
    let pos_before = app.world.get::<Transform>(piece_entity).unwrap().translation;
    
    // Run drag update
    app.world.run_system_once(handle_drag_update);
    
    let pos_after = app.world.get::<Transform>(piece_entity).unwrap().translation;
    let piece_moved = (pos_after.x - pos_before.x).abs() > 0.1 || (pos_after.y - pos_before.y).abs() > 0.1;
    
    println!("   Piece position before: ({:.1}, {:.1})", pos_before.x, pos_before.y);
    println!("   Piece position after: ({:.1}, {:.1})", pos_after.x, pos_after.y);
    ...
    let indicators_after_end = app.world.query::<&ValidMoveIndicator>().iter(&app.world).count();
    
    println!("   Still has Dragging component: {}", still_dragging);
    println!("   Indicators after drag end: {}", indicators_after_end);
    
    assert!(!still_dragging, "Piece should not have Dragging component after release");
