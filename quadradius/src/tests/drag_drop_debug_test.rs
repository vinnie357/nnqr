...
fn test_2d_piece_movement_debug() {
    ...
    println!(
        "   OLD drag bounds (wrong): {:.1} x {:.1}",
        old_drag_bounds, old_drag_bounds
    );
    println!(
        "   NEW drag bounds (fixed): {:.1} x {:.1}",
        new_drag_bounds, new_drag_bounds
    );
    println!(
        "   Size mismatch fixed: {:.1}% -> 0%",
        ((old_drag_bounds - actual_piece_size) / actual_piece_size * 100.0)
    );

    // Test piece bounds for first piece
    let player1_pieces: Vec<_> = pieces_data
        .into_iter()
    ...
    if let Some((entity, piece, transform)) = player1_pieces.first() {
        ...
        println!(
            "   Visual size: {:.1} x {:.1}",
            actual_piece_size, actual_piece_size
        );
        println!(
            "   Click bounds: {:.1} x {:.1}",
            piece_bounds.x, piece_bounds.y
        );
        println!(
            "   Click area: ({:.1} to {:.1}, {:.1} to {:.1})",
            piece_pos.x - piece_bounds.x / 2.0,
            piece_pos.x + piece_bounds.x / 2.0,
            piece_pos.y - piece_bounds.y / 2.0,
            piece_pos.y + piece_bounds.y / 2.0
        );
    }
