...
fn test_3d_movement_indicators_spawn_correctly() {
    ...
    let selected_count = world.query::<(&GamePiece, &Selected)>().iter(&world).count();
    ...
    assert!(indicators_after > 0, "System should spawn movement indicators for selected piece");
    ...
    for &expected_pos in &expected_moves {
        let found = world.query::<&ValidMoveIndicator3D>()
            .iter(&world)
    ...
    assert!(found_moves >= 3, "Should find at least 3 valid movement indicators");
    
    // Verify that indicators have correct components (PbrBundle for 3D rendering)
    println!("\n🔍 Validating 3D rendering components:");
    let indicators_with_pbr = world.query::<(&ValidMoveIndicator3D, &Handle<Mesh>, &Handle<StandardMaterial>)>()
        .iter(&world)
        .count();
    
    println!("📊 Indicators with 3D rendering components: {}", indicators_with_pbr);
    assert_eq!(indicators_with_pbr, indicators_after, "All indicators should have 3D rendering components");
    ...
    assert_eq!(indicators_after_cleanup, 0, "Cleanup system should remove all indicators");
    ...
    println!("  6. ✅ Found {} total movement indicators", indicators_after);
}
...
fn test_3d_indicators_with_movement_powers() {
    ...
    assert!(diagonal_indicators > 0, "Should find diagonal movement indicators when MoveDiagonalActive is present");
    
    println!("✅ Power integration test completed - found {} diagonal indicators", diagonal_indicators);
}
...
fn test_complete_3d_movement_indicator_pipeline() {
    ...
    let test_piece_entity = world.spawn(GamePiece {
        player: Player::Player1,
    ...
    println!("5️⃣ Results: {} movement indicators spawned", total_indicators);
    
    // Validate that we have reasonable number of indicators
    assert!(total_indicators > 0, "Should spawn movement indicators in realistic game scenario");
    assert!(total_indicators <= 8, "Should not spawn more than 8 indicators (max orthogonal + diagonal moves)");
    ...
    assert_eq!(valid_positions, total_indicators, "All indicators should be at valid board positions");
    
    println!("\n✅ Complete Pipeline Test Results:");
    println!("  - 🏁 Game board setup: ✅");
    println!("  - 🎯 Piece selection: ✅");  
    println!("  - 🔧 System execution: ✅");
    println!("  - 📊 Indicator spawning: ✅ ({} indicators)", total_indicators);
    println!("  - 📍 Valid positioning: ✅ ({}/{} valid)", valid_positions, total_indicators);
