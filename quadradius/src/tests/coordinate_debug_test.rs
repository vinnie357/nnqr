...
fn test_coordinate_positioning_debug() {
    ...
    for &(x, y) in &test_positions {
        ...
        println!(
            "    Indicator: ({:6.1}, {:6.1})",
            indicator_world_x, indicator_world_y
        );

        // Check if they're identical
        let piece_tile_match = (piece_world_x - tile_world_x).abs() < 0.01
            && (piece_world_y - tile_world_y).abs() < 0.01;
        let piece_indicator_match = (piece_world_x - indicator_world_x).abs() < 0.01
            && (piece_world_y - indicator_world_y).abs() < 0.01;
    ...
    println!(
        "  Center position should be around ({}, {})",
        center_x_calc - 0.5,
        center_y_calc - 0.5
    );
