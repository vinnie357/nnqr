...
fn test_movement_validation_conditions() {
    ...
    for scenario in scenarios {
        ...
        println!(
            "      From: {:?} -> To: {:?}",
            scenario.start_pos, scenario.end_pos
        );
        println!(
            "      Mouse distance: {:.1} pixels",
            scenario.mouse_distance
        );
        ...
        assert_eq!(
            should_end, scenario.should_end_turn,
            "Movement validation failed for: {}",
            scenario.description
        );

        println!(
            "      ✅ Turn should{} end",
            if scenario.should_end_turn { "" } else { " NOT" }
        );
    }
}
