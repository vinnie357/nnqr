...
fn test_2d_turn_ending_debug_user_scenario() {
    ...
    println!("   Piece at world: ({:.1}, {:.1})", start_world_pos.x, start_world_pos.y);
    
    // Test multiple click scenarios that users might encounter
    let click_scenarios = vec![
        ("Exact center click", start_world_pos),
        ("Slightly off-center (+2px)", Vec2::new(start_world_pos.x + 2.0, start_world_pos.y + 2.0)),
        ("Edge of piece (+15px)", Vec2::new(start_world_pos.x + 15.0, start_world_pos.y)),
        ("Just outside piece (+30px)", Vec2::new(start_world_pos.x + 30.0, start_world_pos.y)),
        ("Far off-center (+50px)", Vec2::new(start_world_pos.x + 50.0, start_world_pos.y + 25.0)),
    ];
    ...
    for (scenario_name, click_pos) in click_scenarios {
        ...
        let piece_position_after = app.world.get::<GamePiece>(piece_entity).unwrap().board_position;
        ...
        match (turn_ended, piece_moved) {
            ...
            (true, false) => {
                ...
                println!("     - World click pos: ({:.1}, {:.1})", click_pos.x, click_pos.y);
                println!("     - Converted board pos: {:?}", world_to_board_position(click_pos));
...
fn test_coordinate_conversion_precision() {
    ...
    let test_cases = vec![
        ...
        ("10px offset", Vec2::new(start_world.x + 10.0, start_world.y)),
        ("20px offset", Vec2::new(start_world.x + 20.0, start_world.y)),
        ("Half tile away", Vec2::new(start_world.x + 38.4, start_world.y)),
    ];
    ...
    for (test_name, world_pos) in test_cases {
        ...
        println!("   {}: world ({:.1}, {:.1})", test_name, world_pos.x, world_pos.y);
        println!("     -> board: {:?}", converted_board);
        println!("     -> back to world: ({:.1}, {:.1})", back_to_world.x, back_to_world.y);
        
        let is_same_board_position = converted_board == start_pos;
        println!("     Same board position as start: {}", is_same_board_position);
        
        if is_same_board_position {
            println!("     ✅ Would correctly NOT end turn (same position)");
        } else {
            println!("     ⚠️ Would end turn (different position: {:?})", converted_board);
        }
        println!();
    }
