...
mod integration_tests {
    ...
    fn test_correct_board_dimensions() {
        ...
        // Original square board size still available for compatibility
        assert_eq!(BOARD_SIZE, 8);
    }

    #[test]
    fn test_all_board_positions_have_theme_colors() {
        // Test that every position on the 10x8 board gets a theme color
    ...
    fn test_piece_positions_on_new_board() {
        // Test that pieces can be placed on all valid positions of 10x8 board
        ...
        // Should have reasonable number of pieces (10 for 10x8 board)
    ...
    fn test_isometric_coordinates_for_rectangular_board() {
        ...
        let width_span = (corner_tr.x - corner_tl.x).abs().max((corner_br.x - corner_bl.x).abs());
        let height_span = (corner_bl.y - corner_tl.y).abs().max((corner_br.y - corner_tr.y).abs());
    ...
    fn test_metallic_theme_consistency() {
        ...
        for color in ui_colors.iter().chain(piece_colors.iter()).chain(orb_colors.iter()) {
            assert_ne!(*color, Color::BLACK);
    ...
    fn test_power_orb_uses_metallic_appearance() {
        ...
        assert!(saturation < 0.5, "Orb base color too saturated for metallic appearance");
