...
fn test_turn_ending_only_on_actual_move() {
    ...
    println!(
        "   ✅ Turn correctly continued (phase: {:?}, player: {:?})",
    ...
    assert_eq!(
        piece_game_piece.board_position, target_board_pos,
        "Piece board position should be updated"
    );

    println!(
        "   ✅ Piece position updated to {:?}",
        piece_game_piece.board_position
    );
...
fn test_drag_to_position(
    ...
    println!(
        "     Drag completed (no longer dragging): {}",
        !still_dragging
    );
...
fn test_multiple_piece_selection_scenarios() {
    ...
    println!(
        "     ✅ Turn ended correctly (advanced to {:?})",
        game_state.turn_phase
    );
