...
fn test_movement_indicator_positioning_matches_piece_bottom() {
    ...
    assert!(
        height_difference < 0.001,
        "Indicator should be positioned at piece bottom height. Difference: {:.4}",
        height_difference
    );
    ...
    assert!(
        clearance_above_tile >= 1.0,
        "Indicators should be well above tile surface for visibility"
    );

    println!(
        "✅ Positioning verified: Indicators at piece bottom height with {:.2} clearance",
        clearance_above_tile
    );
}
