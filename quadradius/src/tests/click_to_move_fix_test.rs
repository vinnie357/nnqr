...
fn test_single_click_should_not_end_turn() {
    ...
    println!(
        "   Min intentional distance: {:.1}",
        min_intentional_distance
    );
    ...
    println!(
        "   Drop world: ({:.1}, {:.1})",
        drop_world_pos.x, drop_world_pos.y
    );

    // Key test: small movements should be below the intentional threshold
    assert!(
        actual_distance < min_intentional_distance,
...
fn test_click_threshold_values() {
    ...
    println!("   Enhanced tile size: {:.1}", enhanced_tile_size);
    println!(
        "   Min intentional distance: {:.1} pixels",
        min_intentional_distance
    );

    // This should be reasonable for distinguishing clicks from drags
    assert!(
        min_intentional_distance > 10.0,
        "Threshold should be more than 10 pixels"
    );
    assert!(
        min_intentional_distance < 50.0,
        "Threshold should be less than 50 pixels"
    );
