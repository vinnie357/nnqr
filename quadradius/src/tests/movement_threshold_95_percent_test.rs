...
fn test_95_percent_movement_threshold() {
    ...
    println!("   Required movement distance: {:.1} pixels ({:.0}%)", 
    ...
    println!("      Should be blocked: {}", small_distance < min_required_distance);
    assert!(small_distance < min_required_distance, 
    ...
    println!("      Should be blocked: {}", medium_distance < min_required_distance);
    assert!(medium_distance < min_required_distance, 
    ...
    println!("      Should be blocked: {}", large_distance < min_required_distance);
    assert!(large_distance < min_required_distance, 
    ...
    println!("      Should be allowed: {}", sufficient_distance >= min_required_distance);
    assert!(sufficient_distance >= min_required_distance, 
    ...
    println!("      Should be allowed: {}", full_tile_distance >= min_required_distance);
    assert!(full_tile_distance >= min_required_distance, 
    ...
    println!("   📏 Threshold: {:.0}% of tile size = {:.1} pixels", 95.0, min_required_distance);
}
...
fn test_practical_drag_scenarios() {
    ...
    for (i, (name, movement, should_allow)) in test_scenarios.iter().enumerate() {
        ...
        println!("\n   {}️⃣ {}:", i + 1, name);
        println!("      Movement: ({:.1}, {:.1}) = {:.1} pixels", 
                 movement.x, movement.y, distance);
        println!("      Expected: {}, Actual: {}", 
                 if *should_allow { "ALLOW" } else { "BLOCK" },
                 if is_allowed { "ALLOW" } else { "BLOCK" });
        
        assert_eq!(is_allowed, *should_allow, 
                   "Movement pattern '{}' validation failed", name);
