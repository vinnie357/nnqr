...
fn test_turn_phase_visual_feedback() {
    ...
    assert_eq!(move_text, "Player 1's Turn (Red) - Move Phase (Click & Drag pieces)");
    ...
    assert_eq!(spawn_text, "Player 1's Turn (Red) - Spawning Phase ⚡ (Wait...)");
    println!("   ✅ PowerSpawning phase: '{}'", spawn_text);
    
    // The key insight: users now see clear "Wait..." message during spawning
    assert!(spawn_text.contains("Wait"), "Spawning phase should clearly indicate waiting");
    assert!(move_text.contains("Click & Drag"), "Move phase should encourage interaction");
}
...
fn test_power_spawning_phase_duration() {
    ...
    println!("   PowerSpawning phase duration: {:.1} seconds", spawning_duration);
    
    // This should be long enough for players to see power orbs spawn
    // but short enough that they don't get frustrated waiting
    assert!(spawning_duration >= 1.0, "Duration should be at least 1 second for visibility");
    assert!(spawning_duration <= 3.0, "Duration should be at most 3 seconds to avoid frustration");
...
fn test_double_click_perception_explanation() {
    ...
    assert!(!can_move_pieces, "Pieces cannot be moved during PowerSpawning");
    ...
    println!("   📝 Explanation: User clicks during spawning (blocked), waits, clicks again (works)");
...
fn test_user_experience_flow() {
    ...
    advance_turn_phase(&mut game_state);
    println!("   2. After power phase: {}", get_phase_display_text(&game_state));
    ...
    // Player moves piece
    advance_turn_phase(&mut game_state);
    println!("   3. After moving: {}", get_phase_display_text(&game_state));
    ...
    advance_turn_phase(&mut game_state);
    println!("   4. Next player's turn: {}", get_phase_display_text(&game_state));
