...
fn test_power_spawning_timer_reset_bug() {
    ...
    println!("   Player1 elapsed time: {:.1} (should not advance)", elapsed);
    ...
    println!("   Player2 should be in PieceMovement: {:?}", game_state.turn_phase);
...
fn test_timer_race_condition() {
    ...
    if elapsed > 2.0 {
        ...
        println!("   Player1 -> Player2, timer reset to: {:?}", timer.start_time);
    }
    
    // Now Player2 goes through their phases
    // PowerActivation -> PieceMovement
    game_state.turn_phase = TurnPhase::PieceMovement;
    println!("   Player2 in PieceMovement, timer should be: {:?}", timer.start_time);
    ...
    if game_state.turn_phase == TurnPhase::PowerSpawning {
        if timer.start_time.is_none() {
            timer.start_time = Some(mock_time);
            println!("   ✅ Player2 timer correctly initialized at: {:.1}", mock_time);
        } else {
...
fn test_bevy_time_resource_behavior() {
    ...
    println!("   Player1 spawning elapsed: {:.1} (should complete)", elapsed_1);
    ...
    println!("   Player2 spawning elapsed: {:.1} (should not complete)", elapsed_2);
    assert!(elapsed_2 <= 2.0);
    
    // Check after delay
    let app_time_4 = 22.5; // 2.5 seconds later
    let elapsed_3 = app_time_4 - timer.start_time.unwrap(); // 2.5 seconds
    println!("   Player2 spawning elapsed: {:.1} (should complete)", elapsed_3);
