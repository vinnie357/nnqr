...
fn test_movement_indicator_shape_matches_board_tiles() {
    ...
    assert_eq!(
        indicator_width, board_tile_width,
        "Indicator width should match board tile width exactly"
    );
    assert_eq!(
        indicator_depth, board_tile_depth,
    ...
    println!(
        "   ✅ Height distinction: {:.2} < {:.2} (indicator < tile)",
        indicator_height, board_tile_height
    );
    ...
    println!(
        "   ✅ Area coverage: {:.1}% (perfect match)",
        coverage_percent
    );
    assert!(
        (coverage_percent - 100.0).abs() < 0.1,
