...
fn test_3d_movement_indicators_with_gamepiece3d() {
    ...
    let selected_3d_count = world.query::<(&GamePiece3D, &Selected)>().iter(&world).count();
    let selected_2d_count = world.query::<(&GamePiece, &Selected)>().iter(&world).count();
    ...
    assert!(final_indicators >= 3 && final_indicators <= 8, 
        "Should spawn 3-8 movement indicators for center piece, got {}", final_indicators);
    
    // Verify indicators have proper 3D rendering components
    let indicators_with_components = world.query::<(&ValidMoveIndicator3D, &Handle<Mesh>, &Handle<StandardMaterial>)>()
        .iter(&world)
        .count();
    assert_eq!(indicators_with_components, final_indicators,
...
fn test_3d_drag_system_compatibility() {
    ...
    let piece_entity = world.spawn(GamePiece3D {
        player: Player::Player1,
        board_position: (3, 4),
    }).id();
    ...
    assert!(indicators > 0, "Should spawn indicators for selected GamePiece3D");
    ...
    assert!(found_moves >= 2, "Should find at least 2 adjacent move indicators");
