...
fn test_2d_movement_overlay_positioning() {
    ...
    // Setup 2D mode
    app.world.get_resource_mut::<RenderConfig>().unwrap().use_3d = false;
    ...
    assert!(
        tile_count >= BOARD_WIDTH as usize * BOARD_HEIGHT as usize,
        "Board tiles should be created"
    );

    println!(
        "   ✅ Test setup verified: {} tiles, {} pieces",
        tile_count, piece_count
    );
}
...
fn test_movement_overlay_coordinate_conversion() {
    ...
    let converted_x =
        ((world_x / enhanced_tile_size) + BOARD_WIDTH as f32 / 2.0 - 0.5).round() as u8;
    let converted_y =
        ((world_y / enhanced_tile_size) + BOARD_HEIGHT as f32 / 2.0 - 0.5).round() as u8;
    ...
    assert_eq!(
        test_board_pos.0, converted_x,
        "X coordinate conversion failed"
    );
    assert_eq!(
        test_board_pos.1, converted_y,
...
fn test_movement_overlay_bounds_validation() {
    ...
    for x in 0..BOARD_WIDTH {
        for y in 0..BOARD_HEIGHT {
            ...
            assert_eq!(
                x, converted_x,
                "X coordinate conversion failed for ({}, {})",
                x, y
            );
            assert_eq!(
                y, converted_y,
                "Y coordinate conversion failed for ({}, {})",
                x, y
            );
        }
    }
...
fn test_movement_overlay_system_fixes() {
    ...
    println!(
        "      - Both board and indicators use enhanced tile size (TILE_SIZE * 1.2 = {})",
        TILE_SIZE * 1.2
    );
    ...
    println!(
        "      - Fixed: show_valid_moves_for_powers_3d now queries GamePiece instead of BoardTile"
    );
    ...
    println!(
        "      - 2D uses enhanced_tile_size = TILE_SIZE * 1.2 = {}",
        TILE_SIZE * 1.2
    );
    println!(
        "      - 3D uses TILE_SIZE_MULTIPLIER_3D = 1.5 = {}",
        TILE_SIZE * 1.5
    );
    ...
    assert_eq!(
        test_pos.0, back_x,
        "Coordinate conversion should be perfect"
    );
    assert_eq!(
        test_pos.1, back_y,
        "Coordinate conversion should be perfect"
