...
fn test_selected_component_removed_after_drag() {
    ...
    assert!(app.world.get::<Selected>(piece_entity).is_some(), 
            "Piece should be selected before drag end");
    assert!(app.world.get::<Dragging>(piece_entity).is_some(), 
    ...
    assert!(app.world.get::<Dragging>(piece_entity).is_none(), 
            "Dragging component should be removed after drag end");
    assert!(app.world.get::<Selected>(piece_entity).is_none(), 
...
fn test_drag_drop_component_cleanup_verification() {
    ...
    assert!(should_treat_as_moved, "Should be treated as actual movement");
