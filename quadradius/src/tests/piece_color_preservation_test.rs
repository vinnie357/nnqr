...
mod tests {
    ...
    fn test_piece_colors_preserved_after_invalid_move_flash() {
        ...
        let player1_piece = app.world.spawn((
            GamePiece {
        ...
        let elapsed_time = 0.5; // Greater than duration (0.3)
        ...
        app.world.entity_mut(player1_piece).get_mut::<InvalidMoveFlash>().unwrap().start_time = -0.5; // Trick: make elapsed = 0.5
        ...
        assert!(
            app.world.entity(player1_piece).get::<InvalidMoveFlash>().is_none(),
    ...
    fn test_flash_color_interpolation_uses_correct_base() {
        ...
        let test_piece = app.world.spawn((
            GamePiece {
                player: Player::Player1,
                board_position: (2, 2),
            },
        ...
        app.world.resource_mut::<Time>().advance_by(std::time::Duration::from_millis(150)); // Half duration
