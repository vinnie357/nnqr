...
fn test_capture_validation_logic() {
    ...
    assert!(
        !blocked_by_friendly,
        "Should not be blocked by friendly piece"
    );
    ...
    assert!(
        enemy_piece_present,
        "Enemy piece should be present at target"
    );
    ...
    assert!(
        is_valid_distance,
        "Move should be valid distance (adjacent)"
    );
...
fn test_enhanced_movement_capture_logic() {
    ...
    assert!(
        blocked_friendly,
        "Enhanced validation should block friendly capture"
    );
