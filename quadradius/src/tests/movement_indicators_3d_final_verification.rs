...
fn test_3d_movement_indicators_issue_resolution() {
    ...
    let selected_piece = world.spawn((
        GamePiece {
    ...
    let selected_pieces = world.query::<(&GamePiece, &Selected)>().iter(&world).count();
    ...
    assert_eq!(initial_indicators, 0, "Should start with no movement indicators");
    ...
    assert!(final_indicators > 0, "❌ CRITICAL FAILURE: No movement indicators spawned! The issue persists.");
    ...
    for &pos in &expected_positions {
        ...
        if found {
            ...
            } else if pos != (2, 1) { // Skip friendly occupied position
                valid_moves_found += 1;
    ...
    assert!(valid_moves_found >= 3, "Should have at least 3 valid move indicators");
    ...
    let indicators_with_3d_components = world.query::<(&ValidMoveIndicator3D, &Handle<Mesh>, &Handle<StandardMaterial>)>()
        .iter(&world)
        .count();
    
    assert_eq!(indicators_with_3d_components, final_indicators, 
    ...
    assert_eq!(cleanup_indicators, 0, "Cleanup should remove all indicators");
    ...
    println!("   - Movement indicator spawning: ✅ {} indicators spawned", final_indicators);
    println!("   - 3D rendering components: ✅ All indicators have PbrBundle");
    println!("   - Attack move detection: 📊 {} attack indicators", attack_moves_found);
    println!("   - Valid move detection: ✅ {} valid indicators", valid_moves_found);
...
fn test_3d_indicators_with_enhanced_powers_integration() {
    ...
    println!("📊 Total indicators with diagonal power: {}", total_indicators);
    
    // Should have 8 indicators: 4 orthogonal + 4 diagonal
    assert!(total_indicators >= 8, "Should have at least 8 indicators with diagonal movement power");
    ...
    assert!(diagonal_found >= 4, "Should find all 4 diagonal movement indicators");
    println!("✅ Enhanced power integration working: {} diagonal indicators found", diagonal_found);
}