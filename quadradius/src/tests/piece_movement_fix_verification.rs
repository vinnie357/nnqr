...
fn test_2d_piece_movement_fix_comprehensive() {
    ...
    let piece_positions = vec![
        ...
        (4, 0),
        (6, 0),
        (8, 0), // Player 1 bottom row
        (1, 1),
        (3, 1),
        (5, 1),
        (7, 1),
        (9, 1), // Player 1 second row
        (0, 6),
        (2, 6),
        (4, 6),
        (6, 6),
        (8, 6), // Player 2 top rows
        (1, 7),
        (3, 7),
        (5, 7),
        (7, 7),
    ...
    println!(
        "   Visual piece size: {:.1} x {:.1}",
        piece_visual_size, piece_visual_size
    );
    println!(
        "   Hitbox size (fixed): {:.1} x {:.1}",
        fixed_hitbox_size, fixed_hitbox_size
    );
    ...
    let player1_pieces: Vec<_> = app
        .world
        .query::<(Entity, &GamePiece, &Transform)>()
        .iter(&app.world)
    ...
    if let Some((entity, piece, transform)) = player1_pieces.first() {
        ...
        println!(
            "   Clickable area: ({:.1} to {:.1}, {:.1} to {:.1})",
            piece_pos.x - half_size,
            piece_pos.x + half_size,
            piece_pos.y - half_size,
            piece_pos.y + half_size
        );
    ...
    assert!(
        can_move,
        "Should be able to move pieces in PieceMovement phase"
    );
    assert!(
        current_player_pieces > 0,
        "Should have pieces for current player"
    );
    ...
    let camera_count = app
        .world
        .query_filtered::<Entity, (With<crate::systems::settings::Camera2D>, With<Camera>)>()
        .iter(&app.world)
        .count();
...
fn test_2d_movement_edge_cases() {
    ...
    for (x, y) in test_positions {
        ...
        let coord_ok = world_x.is_finite()
            && world_y.is_finite()
            && world_x.abs() < 10000.0
            && world_y.abs() < 10000.0;
        ...
        assert!(
            coord_ok,
            "Coordinates should be valid for position ({}, {})",
            x, y
        );
    }
    ...
    for (distance, desc, should_hit) in boundary_tests {
        ...
        if hits == should_hit {
            ...
            println!(
                "   ❌ Distance {:.1} [{}]: {} (wrong, expected {})",
                distance, desc, hits, should_hit
            );
        }
