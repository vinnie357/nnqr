...
fn test_board_tile_movement_indicator_alignment() {
    ...
    let mut indicator_positions: Vec<(u8, u8)> = world.query::<&ValidMoveIndicator3D>()
        .iter(&world)
    ...
    assert!(all_positions_valid, "All movement indicators should correspond to valid board tile positions");
    
    // Test coordinate conversion consistency
    println!("\n📐 Testing coordinate conversion consistency:");
    for &pos in indicator_positions.iter().take(3) { // Test first 3 positions
        let board_tile_pos = board_to_isometric(pos, 0.0);
        println!("   Board({}, {}) -> Isometric({:.2}, {:.2}, {:.2})", 
    ...
    println!("   - All indicators at valid board positions: {}", all_positions_valid);
    println!("   - Coordinate system consistent: board_to_isometric() used by both");
    
    assert!(indicator_positions.len() > 0, "Should find movement indicators");
    assert!(indicator_positions.len() <= 8, "Should not exceed maximum possible moves");
}
...
fn test_visual_vs_logical_grid_alignment() {
    ...
    for &(x, y) in &critical_positions {
        ...
        println!("   Board({}, {}) -> Visual({:.1}, {:.1}, {:.1}) [{}]", 
                 x, y, visual_pos.x, visual_pos.y, visual_pos.z,
        ...
        if within_bounds {
            ...
            assert!(y < BOARD_HEIGHT, "Y coordinate should be within board height");
        }
    }
    ...
    println!("   Logical center: ({:.1}, {:.1})", board_center_x, board_center_y);
    ...
    for &tile in &center_tiles {
        ...
        println!("     {:?} -> ({:.1}, {:.1}, {:.1})", tile, pos.x, pos.y, pos.z);
    }
...
fn test_movement_indicator_board_mismatch_diagnosis() {
    ...
    for i in 0..test_positions.len() {
        ...
        println!("   {:?} -> ({:.2}, {:.2}, {:.2})", pos, world_pos.x, world_pos.y, world_pos.z);
        ...
        if i > 0 {
            ...
            let distance = ((world_pos.x - prev_world.x).powi(2) + 
                           (world_pos.z - prev_world.z).powi(2)).sqrt();
