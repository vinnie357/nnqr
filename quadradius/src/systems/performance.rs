...
pub fn display_performance_stats(
    ...
    if keyboard.just_pressed(KeyCode::F8) {
        ...
        if let Some(&current_entities) = monitor.entity_counts.back() {
            ...
            let avg_entities = monitor.entity_counts.iter().sum::<usize>() as f32 / monitor.entity_counts.len() as f32;
        ...
        if !monitor.frame_times.is_empty() {
            let avg_frame_time = monitor.frame_times.iter().sum::<f32>() / monitor.frame_times.len() as f32;
            let min_frame_time = monitor.frame_times.iter().cloned().fold(f32::INFINITY, f32::min);
            let max_frame_time = monitor.frame_times.iter().cloned().fold(0.0, f32::max);
            
            println!("⏱️  Frame Times:");
            println!("   Average: {:.2}ms ({:.1} fps)", avg_frame_time * 1000.0, 1.0 / avg_frame_time);
            println!("   Best: {:.2}ms ({:.1} fps)", min_frame_time * 1000.0, 1.0 / min_frame_time);
            println!("   Worst: {:.2}ms ({:.1} fps)", max_frame_time * 1000.0, 1.0 / max_frame_time);
        }
...
pub fn cleanup_entities(
    ...
    old_particles: Query<Entity, (With<crate::systems::visual_effects::ParticleEffect>, With<PerformanceOptimized>)>,
    old_indicators: Query<Entity, (With<crate::systems::power_effects::PowerTargetIndicator>, With<PerformanceOptimized>)>,
    old_effects: Query<Entity, (With<crate::systems::power_effects::ActivePowerEffect>, With<PerformanceOptimized>)>,
) {
            ...
            if fps < monitor.target_fps as f64 * 0.6 {
                ...
                if particle_count > 30 {
                    for (i, entity) in particles.iter().enumerate() {
                        if i >= 20 { // Keep only 20 particles
                            commands.entity(entity).despawn();
                        }
                    }
                }
