...
fn adjust_spawn_rates_by_usage(
    config: &mut PowerBalanceConfig,
    tracker: &PowerUsageTracker,
) {
    for (power_type, usage_count) in &tracker.usage_counts {
        let current_rate = config.power_spawn_rates.get(power_type).copied().unwrap_or(0.5);
        ...
        println!("   {:?}: {:.2} -> {:.2} (usage: {}, success: {:.1}%)", 
            power_type, current_rate, new_rate, usage_count, success_rate * 100.0);
    }
}

fn adjust_spawn_rates_adaptive(
    config: &mut PowerBalanceConfig,
    tracker: &PowerUsageTracker,
) {
    ...
    if total_usage > 0 {
        for (power_type, usage_count) in &tracker.usage_counts {
            ...
            config.power_spawn_rates.insert(*power_type, new_rate.clamp(0.05, 1.0));
        }
    }
}
...
pub fn spawn_balanced_power_orbs(
    ...
    println!("Turn {} - checking balanced power orb spawn for {:?}", 
    ...
    if rand::random::<f32>() < spawn_chance {
        ...
        let occupied_positions: std::collections::HashSet<_> = existing_orbs.iter()
            .map(|orb| orb.board_position)
            .collect();
        
        let available_positions: Vec<_> = board_tiles.iter()
            .filter(|tile| !occupied_positions.contains(&tile.coordinates))
            .collect();
        
        if !available_positions.is_empty() {
            let spawn_pos = available_positions[rand::thread_rng().gen_range(0..available_positions.len())];
            ...
            println!("Balanced power orb spawned: {:?} at ({}, {}) [spawn rate: {:.1}%]", 
                power_type, spawn_pos.coordinates.0, spawn_pos.coordinates.1, 
                balance_config.power_spawn_rates.get(&power_type).unwrap_or(&0.5) * 100.0);
        }
    }
}
...
pub fn track_power_usage_for_balance(
    ...
    for event in power_events.read() {
        // Update usage count
        *usage_tracker.usage_counts.entry(event.power_type).or_insert(0) += 1;
        
        // Update success rate
        let entry = usage_tracker.success_rates.entry(event.power_type).or_insert((0, 0));
        ...
        usage_tracker.last_used.insert(event.power_type, turn_counter.turn_number);
        
        // Update game usage count
        *usage_tracker.game_usage_limits.entry(event.power_type).or_insert(0) += 1;
    }
}

fn print_balance_changes(config: &PowerBalanceConfig) {
    println!("\n📊 CURRENT BALANCE SETTINGS:");
    println!("   Global spawn rate: {:.1}%", config.global_spawn_rate * 100.0);
