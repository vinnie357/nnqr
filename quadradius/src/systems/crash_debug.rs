...
fn test_null_access(tiles: &Query<&BoardTile>) {
    ...
    for &coords in &test_coords {
        ...
        println!("     Tile at {:?}: {}", coords, if found { "exists" } else { "missing" });
    }
}
...
pub fn validate_game_safety(
    ...
    for piece in pieces.iter() {
        if piece.board_position.0 >= 8 || piece.board_position.1 >= 8 {
            println!("⚠️ SAFETY: Piece found outside board bounds: {:?}", piece.board_position);
        }
    }
    ...
    for piece in pieces.iter() {
        if !positions.insert(piece.board_position) {
            println!("⚠️ SAFETY: Duplicate pieces at position: {:?}", piece.board_position);
        }
    }
    ...
    if let Some(power_index) = game_state.selected_power {
        ...
        if power_index >= powers.len() {
            println!("⚠️ SAFETY: Invalid power index {} (max: {})", power_index, powers.len());
        }
    }
    ...
    if tile_count != 64 {
        println!("⚠️ SAFETY: Incorrect tile count: {} (expected 64)", tile_count);
    }
}

// Entity cleanup validator to prevent double-despawn warnings
pub fn validate_entity_cleanup(
    mut removed: RemovedComponents<GamePiece>,
    mut commands: Commands,
) {
...
pub fn detect_performance_crashes(
    ...
    if let Some(fps_diagnostic) = diagnostics.get(bevy::diagnostic::FrameTimeDiagnosticsPlugin::FPS) {
        if let Some(fps) = fps_diagnostic.smoothed() {
            if fps < 5.0 {
                println!("🚨 CRITICAL: FPS dropped to {} - potential freeze/crash incoming!", fps);
            } else if fps < 15.0 {
