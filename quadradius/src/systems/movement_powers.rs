...
pub fn activate_swap(
    ...
    if let (Some((_source_entity, source_pos, _)), Some((_target_entity, target_pos, _))) = (source_piece, target_piece) {
...
pub fn activate_slide(
    ...
    for (_entity, piece) in pieces.iter() {
        if piece.player == game_state.current_player {
            ...
            println!("Slide power activated for player {}'s pieces", 
