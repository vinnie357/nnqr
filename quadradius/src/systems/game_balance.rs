...
pub fn track_power_usage(
    ...
    for event in power_events.read() {
        // Update usage count
        *balance_data.power_usage_count.entry(event.power_type).or_insert(0) += 1;
        
        // Update success rate
        let entry = balance_data.power_success_rate.entry(event.power_type).or_insert((0, 0));
        if event.was_successful {
            entry.0 += 1;
        }
        entry.1 += 1;
        
        println!("Power used: {:?} by {:?} - Success: {} (Turn {})", 
...
pub fn track_game_completion(
    ...
    for event in game_events.read() {
        ...
        if balance_data.shortest_game_turns == 0 || balance_data.shortest_game_turns > event.turn_count {
            balance_data.shortest_game_turns = event.turn_count;
        }
        
        println!("Game completed - Winner: {:?}, Duration: {:.1}s, Turns: {}", 
            event.winner, event.game_duration, event.turn_count);
    }
}

// Generate balance report
pub fn generate_balance_report(
    ...
    if keyboard.just_pressed(KeyCode::F9) {
        ...
        for (power, count) in usage_vec.iter().take(10) {
            let success_rate = if let Some((successes, total)) = balance_data.power_success_rate.get(power) {
                if *total > 0 {
            ...
            println!("  {:?}: {} uses ({:.1}% success rate)", power, count, success_rate);
        }
        
        // Game length statistics
        if !balance_data.game_length_samples.is_empty() {
            let avg_duration = balance_data.game_length_samples.iter().sum::<f32>() / balance_data.game_length_samples.len() as f32;
            let avg_turns = balance_data.turn_count_samples.iter().sum::<u32>() as f32 / balance_data.turn_count_samples.len() as f32;
            ...
            println!("  Shortest game: {} turns", balance_data.shortest_game_turns);
        }
        
        // Win rate balance
        let total_games = balance_data.player_win_counts[0] + balance_data.player_win_counts[1];
        if total_games > 0 {
            let p1_winrate = (balance_data.player_win_counts[0] as f32 / total_games as f32) * 100.0;
            let p2_winrate = (balance_data.player_win_counts[1] as f32 / total_games as f32) * 100.0;
            
            println!("\nWIN RATE BALANCE:");
            println!("  Player 1: {}% ({}/{})", p1_winrate, balance_data.player_win_counts[0], total_games);
            println!("  Player 2: {}% ({}/{})", p2_winrate, balance_data.player_win_counts[1], total_games);
    ...
    if keyboard.just_pressed(KeyCode::F10) {
        ...
        for (power, (successes, total)) in &balance_data.power_success_rate {
            if *total > 0 {
                ...
                let usage_frequency = *balance_data.power_usage_count.get(power).unwrap_or(&0) as f32;
        ...
        for (i, (power, score)) in effectiveness_scores.iter().enumerate() {
            ...
            let (successes, total) = balance_data.power_success_rate.get(power).unwrap_or(&(0, 0));
            let success_rate = if *total > 0 { *successes as f32 / *total as f32 * 100.0 } else { 0.0 };
            
            println!("  {}. {:?}: {:.2} effectiveness ({} uses, {:.1}% success)", 
                i + 1, power, score, usage, success_rate);
        }
    ...
    if keyboard.just_pressed(KeyCode::F11) {
        ...
        if !balance_data.power_spawn_locations.is_empty() {
            ...
            for (i, ((x, y), count)) in sorted_spawns.iter().take(5).enumerate() {
                ...
                println!("  {}. ({}, {}): {} spawns ({:.1}%)", i + 1, x, y, count, percentage);
            }
            ...
            if let Some((_, max_spawns)) = sorted_spawns.first() {
                if let Some((_, min_spawns)) = sorted_spawns.last() {
                    ...
                    if imbalance_ratio > 3.0 {
                        println!("  WARNING: Significant spawn imbalance detected ({}x difference)", imbalance_ratio);
                    } else if imbalance_ratio > 2.0 {
                        println!("  CAUTION: Moderate spawn imbalance ({}x difference)", imbalance_ratio);
                    } else {
    ...
    if keyboard.just_pressed(KeyCode::F12) {
        ...
        if total_games > 10 {
            ...
            if (p1_winrate - 0.5).abs() > 0.15 {
                recommendations.push("🔧 CRITICAL: Adjust starting positions or first-turn advantage".to_string());
            } else if (p1_winrate - 0.5).abs() > 0.1 {
                recommendations.push("⚠️  Consider minor balance adjustments for player parity".to_string());
            }
        }
        ...
        if !balance_data.power_usage_count.is_empty() {
            ...
            if max_usage > &0 && *max_usage / (min_usage + 1) > 5 {
                recommendations.push("🔧 Some powers are heavily overused while others are ignored".to_string());
                recommendations.push("   → Consider buffing weak powers or nerfing dominant ones".to_string());
            }
        }
        ...
        for (power, (successes, total)) in &balance_data.power_success_rate {
            if *total > 5 {
                let success_rate = *successes as f32 / *total as f32;
                if success_rate < 0.3 {
                    recommendations.push(format!("🔧 {:?} has low success rate ({:.1}%) - consider buffing", power, success_rate * 100.0));
                } else if success_rate > 0.9 {
                    recommendations.push(format!("⚠️  {:?} has very high success rate ({:.1}%) - might be overpowered", power, success_rate * 100.0));
                }
            }
        }
        ...
        if !balance_data.turn_count_samples.is_empty() {
            let avg_turns = balance_data.turn_count_samples.iter().sum::<u32>() as f32 / balance_data.turn_count_samples.len() as f32;
            if avg_turns < 10.0 {
                recommendations.push("⚠️  Games are very short - consider slowing down win conditions".to_string());
            } else if avg_turns > 50.0 {
                recommendations.push("⚠️  Games are very long - consider accelerating win conditions".to_string());
            }
        }
