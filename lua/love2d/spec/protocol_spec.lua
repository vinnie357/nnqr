...
describe("Protocol", function()
	...
	describe("AI game messages", function()
		...
		describe("opponentDisconnectedMessage", function()
			it("creates an OPPONENT_DISCONNECTED message", function()
				local msg = Protocol.opponentDisconnectedMessage("game_123", "Bob", 60)
				assert.are.equal("OPPONENT_DISCONNECTED", msg.type)
				assert.are.equal("game_123", msg.payload.game_id)
				assert.are.equal("Bob", msg.payload.opponent_name)
				assert.are.equal(60, msg.payload.reconnect_timeout)
			end)
		end)

		describe("opponentReconnectedMessage", function()
			it("creates an OPPONENT_RECONNECTED message", function()
				local msg = Protocol.opponentReconnectedMessage("game_123", "Bob")
				assert.are.equal("OPPONENT_RECONNECTED", msg.type)
				assert.are.equal("game_123", msg.payload.game_id)
				assert.are.equal("Bob", msg.payload.opponent_name)
			end)
		end)
	...
	describe("message types - disconnect", function()
		it("has OPPONENT_DISCONNECTED type", function()
			assert.are.equal("OPPONENT_DISCONNECTED", Protocol.Types.OPPONENT_DISCONNECTED)
		end)

		it("has OPPONENT_RECONNECTED type", function()
			assert.are.equal("OPPONENT_RECONNECTED", Protocol.Types.OPPONENT_RECONNECTED)
		end)
	end)
end)
