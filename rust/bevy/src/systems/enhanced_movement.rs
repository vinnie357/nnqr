...
pub fn validate_enhanced_movement(
    ...
    // Check if target is an acid hole (permanently impassable)
    // TODO: Add acid_hole_query parameter to function signature
    // For now, check terrain height - acid holes are set to MIN_HEIGHT - 1
    for tile in tiles.iter() {
        if tile.coordinates == to && tile.height < crate::systems::terrain_height::MIN_HEIGHT {
            return false; // Cannot move to acid hole
        }
    }
