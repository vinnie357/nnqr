...
/// Check if current player has MoveTwice pieces with remaining moves
pub fn check_move_twice_remaining(
...
pub fn handle_move_twice_move(
    mut commands: Commands,
    moved_entity: Entity,
    mut move_twice_pieces: Query<(Entity, &mut MoveTwiceActive, &GamePiece)>,
    current_player: Player,
) -> bool {
    // Check if the moved piece has MoveTwice active
    if let Ok((_, mut move_twice, piece)) = move_twice_pieces.get_mut(moved_entity) {
        if piece.player == current_player && move_twice.moves_remaining > 0 {
            move_twice.moves_remaining -= 1;
            info!(
