...
pub fn update_height_sprite_colors(
    ...
    for (_, mut sprite, terrain) in tiles.iter_mut() {
        ...
        let _height_factor = (terrain.height + 2) as f32 / 7.0; // Normalize to 0-1
...
/// Get the current height of a tile at the given coordinates
pub fn get_tile_height(
    x: u8,
    y: u8,
    tiles: &Query<(Entity, &mut BoardTile, &mut TerrainHeight)>,
) -> i8 {
    for (_, tile, _) in tiles.iter() {
        if tile.coordinates == (x, y) {
            return tile.height;
        }
    }
    0 // Default height if tile not found
}
