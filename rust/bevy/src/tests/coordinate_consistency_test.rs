...
fn test_coordinate_system_consistency() {
    ...
    for &(x, y) in &test_positions {
        ...
        println!(
            "  Board({}, {}) -> World({:.1}, {:.1})",
            x, y, expected_world_x, expected_world_y
        );
    }
    ...
    assert_eq!(
        ...
        "Enhanced tile size should be 76.8"
    );
    assert_eq!(TILE_SIZE, 64.0, "Base tile size should be 64.0");
    assert_eq!(
        enhanced_tile_size / TILE_SIZE,
        1.2,
        "Enhancement multiplier should be 1.2"
    );
