...
fn test_chat_starts_minimized_in_2d_view() {
    ...
    assert!(
        chat_state.is_minimized,
        "Chat should start minimized by default"
    );
    ...
    for (style, _) in panel_query.iter(&app.world) {
        ...
        match style.height {
            Val::Px(height) => {
                assert_eq!(
                    height, 50.0,
                    "Chat panel should have minimized height of 50px"
                );
    ...
    for (visibility, _) in content_query.iter(&app.world) {
        ...
        assert_eq!(
            *visibility,
            Visibility::Hidden,
            "Chat content area should be hidden when minimized"
        );
    ...
    let mut button_query = app
        .world
        .query_filtered::<Entity, With<ChatMinimizeButton>>();
    let button_entities: Vec<Entity> = button_query.iter(&app.world).collect();
    assert_eq!(
        button_entities.len(),
        1,
        "Should have exactly one minimize button"
    );

    let button_entity = button_entities[0];
    let children = app
        .world
        .get::<Children>(button_entity)
        .expect("Button should have children");
    let text_entity = children[0];
    let text = app
        .world
        .get::<Text>(text_entity)
        .expect("Button child should have Text component");

    assert_eq!(
        text.sections[0].value, "⬜",
        "Minimize button should show maximize symbol when chat is minimized"
    );
    println!(
        "   ✅ Minimize button symbol: '{}' (maximize)",
        text.sections[0].value
    );
...
fn test_chat_toggle_in_2d_view() {
    ...
    assert!(
        !chat_state.is_minimized,
        "Chat should be maximized after click"
    );
    ...
    for (visibility, _) in content_query.iter(&app.world) {
        assert_eq!(
            *visibility,
            Visibility::Visible,
            "Content should be visible when maximized"
        );
    ...
    assert!(
        chat_state.is_minimized,
        "Chat should be minimized after second click"
    );
...
fn test_chat_consistent_across_view_switches() {
    ...
    assert!(
        chat_state.is_minimized,
        "Chat state should persist when switching to 3D"
    );
    ...
    assert!(
        !chat_state.is_minimized,
        "Chat state should persist when switching back to 2D"
    );
