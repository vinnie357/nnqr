...
fn test_chat_starts_minimized() {
    ...
    assert!(
        chat_default.is_minimized,
        "Chat should start minimized when using Default trait"
    );

    // Test using new() method
    let chat_new = ChatState::new();
    assert!(
        chat_new.is_minimized,
        "Chat should start minimized when using new() method"
    );

    // Verify other default values
    assert_eq!(
        chat_default.messages.len(),
        0,
        "Should start with no messages"
    );
    assert_eq!(
        chat_default.current_input, "",
        "Should start with empty input"
    );
    assert_eq!(
        chat_default.max_messages, 100,
        "Should have max_messages set to 100"
    );
    assert_eq!(
        chat_default.unread_count, 0,
        "Should start with no unread messages"
    );
    ...
    assert_eq!(
        chat.unread_count, 1,
        "Should have 1 unread message after adding while minimized"
    );
    println!(
        "   ✅ Unread count increments when minimized: {}",
        chat.unread_count
    );
    ...
    assert!(
        !chat.is_minimized,
        "Chat should be maximized after calling maximize()"
    );
    assert_eq!(
        chat.unread_count, 0,
        "Unread count should be cleared when maximized"
    );
    println!(
        "   ✅ Maximizing clears unread count: {}",
        chat.unread_count
    );
...
fn test_init_resource_pattern() {
    ...
    assert!(
        ...
        "ChatState from init_resource should start minimized"
    );
