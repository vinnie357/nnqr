...
fn test_movement_indicators_align_with_visual_grid() {
    ...
    for &test_pos in &test_positions {
        ...
        let indicator_positions: Vec<(u8, u8)> = world
        ...
        let indicators: Vec<Entity> = world
            .query::<Entity>()
            .iter(&world)
...
fn test_spacing_discrepancy_resolution() {
    ...
    let discrepancy_percent = ((actual_distance - expected_isometric_distance).abs()
        / expected_isometric_distance)
        * 100.0;
    ...
    assert!(
        ...
        "Spacing discrepancy should be resolved to under 5%, but was {:.1}%",
        discrepancy_percent
    );
    ...
    assert!(
        size_match,
        "Visual tile size should match coordinate spacing"
    );

    println!(
        "✅ Spacing issue resolved: {:.1}% discrepancy (previously 11.4%)",
        discrepancy_percent
    );
}
...
fn test_grid_alignment_issue_resolution() {
    ...
    let indicators: Vec<(u8, u8)> = world
        .query::<&ValidMoveIndicator3D>()
        .iter(&world)
        .map(|indicator| indicator.coordinates)
        .collect();
    ...
    println!(
        "✅ Found {} valid adjacent moves (expected up to 4)",
        valid_adjacent_moves
    );
    assert!(
        valid_adjacent_moves > 0,
        "Should find at least one valid adjacent move"
    );
    assert!(
        valid_adjacent_moves <= 4,
        "Should not exceed 4 orthogonal moves"
    );
    ...
    for &pos in &indicators {
        ...
        assert!(
            tile_exists,
            "Indicator should be at a valid board tile position"
        );
    }
