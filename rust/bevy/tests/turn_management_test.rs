...
fn test_clicking_piece_without_moving_does_not_end_turn() {
    ...
    // Add input resources needed by systems
    ...
    app.world
        ...
        .insert(quadradius::components::piece::Dragging {
            offset: Vec2::ZERO,
    ...
    app.world
        .entity_mut(piece_entity)
        .remove::<quadradius::components::piece::Dragging>();
    app.insert_resource(bevy::input::Input::<MouseButton>::default());
...
fn test_moving_piece_to_new_square_ends_turn() {
    ...
    // Add 2D camera for drag_drop system
    app.world.spawn((
        Camera2dBundle::default(),
        quadradius::systems::settings::Camera2D,
    ));
    ...
    // Manually advance turn phase to simulate successful move
    {
        let mut game_state = app.world.resource_mut::<GameState>();
        quadradius::systems::turn_management::advance_turn_phase(&mut game_state);
    }
...
fn test_raycast_selection_does_not_interfere_with_drag() {
    ...
    app.world
        .entity_mut(piece_entity)
        .insert(Dragging3D { start_pos: (0, 0) });
    ...
    let dragging_count = app.world.query::<&Dragging3D>().iter(&app.world).count();
...
fn test_raycast_only_selects_not_drags() {
    ...
    let has_dragging = app.world.get::<Dragging3D>(piece_entity).is_some();
    ...
    let has_selected = app.world.get::<Selected>(piece_entity).is_some();
